...
namespace MovieManager.PL.API.Controllers
{
    ...
    public class ReviewsController : ControllerBase
    {
        ...
        public async Task<ActionResult<ReviewModel>> Create(ReviewModel model, CancellationToken cancellationToken)
        {
        ...
        [ProducesResponseType(StatusCodes.Status400BadRequest)]
        [ProducesResponseType(StatusCodes.Status404NotFound)]
        public async Task<IActionResult> Update(int id, ReviewModel model, CancellationToken cancellationToken)
        {
            if (id != model.Id)
                return BadRequest();
