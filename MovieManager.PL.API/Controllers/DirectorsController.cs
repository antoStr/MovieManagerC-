...
namespace MovieManager.PL.API.Controllers
{
    ...
    public class DirectorsController : ControllerBase
    {
        ...
        public async Task<ActionResult<DirectorModel>> Create(DirectorModel model, CancellationToken cancellationToken)
        {
        ...
        [ProducesResponseType(StatusCodes.Status400BadRequest)]
        [ProducesResponseType(StatusCodes.Status404NotFound)]
        public async Task<IActionResult> Update(int id, DirectorModel model, CancellationToken cancellationToken)
        {
            if (id != model.Id)
                return BadRequest();
